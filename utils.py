...
def load_wav_to_torch(full_path):
  ...
  if sampling_rate!=16000:
    new_rate = 16000
    num_new_samples = int(round(len(data) * float(new_rate) / sampling_rate))
    x = resample(data, num_new_samples).astype(np.int16)
    return torch.FloatTensor(x.astype(np.float32)), 16000
  else:
